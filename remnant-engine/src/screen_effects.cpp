...
ScreenEffects::ScreenEffects() :
    _fade_alphas(),
    _fade_hbe(re::blending_fade_alpha_hbe_ptr::create(_fade_alphas))
{
...
{
    if (re::keypad::a_pressed()) { next_state(); }
...
void ScreenEffects::set_camera_blend()
{
    ...
    int amplitude = re::display::height() / 2;

    re::array<re::blending_fade_alpha, re::display::height()> friendly;

    for(int i = 0; i < amplitude; ++i)
    {
        re::fixed alpha = re::fixed(i) / amplitude;
...
void ScreenEffects::blend_array_with_fade_alphas(
    re::array<re::blending_fade_alpha, re::display::height()>
    friendly)
{
    for (int i = 0; i < re::display::height(); ++i) {
        re::fixed alpha = (_fade_alphas[i].value() + friendly[i].value()) / 2;
...
{
    for(int i = 0; i < re::display::height(); i += 2)
    {
        _fade_alphas[i].set_value(re::fixed(0.3));
...
void ScreenEffects::fade_out_update()
{
    if(_current_frame <= _duration_frames) {
        re::fixed alpha = re::fixed(_current_frame)/_duration_frames;
...
void ScreenEffects::fade_in_camera_update() {
    ...
    re::array<re::blending_fade_alpha, re::display::height()> butano;
    ...
    for(int i = 0; i < re::display::height(); i += 2)
    {
        re::fixed alpha = (butano[i].value() + 0.3) / 2;
